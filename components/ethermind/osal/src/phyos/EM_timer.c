...
//extern uint8 osal_event_search( uint8 task_id, uint16 event_flag );
...
//			printf("-3-");
////			hal_gpio_write(P13,1);
////			hal_gpio_write(P13,0);
//		}
...
#if 1
Status_t osal_CbTimerUpdate32( uint8 timerId, uint32 timeout )
{
  // Look for the existing timer
  if ( timerId < NUM_CBTIMERS32 )
  {
    if ( cbTimers[timerId].pfnCbTimer != NULL )
    {
      // Make sure the corresponding OSAL event timer is still running
      if ( osal_get_timeoutEx( TASK_ID32( timerId ), EVENT_ID32( timerId ) ) != 0 )
      {
        // Timer exists; update it
        osal_start_timerEx( TASK_ID32( timerId ), EVENT_ID32( timerId ), timeout );
        return (  SUCCESS );
      }
...
)
{
    ...
    {	
        if((osal_CbTimerUpdate32(timer_id,new_timeout) == EM_SUCCESS)
			|| (osalFindTimer(TASK_ID32( timer->timer_id ),EVENT_ID32( timer->timer_id )) == NULL))
...
)
{
    ...
    {
        if(osal_CbTimerUpdate32(timer->timer_id,new_timeout) == EM_SUCCESS)
        {
            return ( SUCCESS );
        }
    }
