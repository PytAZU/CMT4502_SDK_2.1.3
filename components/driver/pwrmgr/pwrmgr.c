...
#elif(CFG_SLEEP_MODE == PWR_MODE_PWROFF)
static uint8_t mPwrMode = PWR_MODE_PWROFF;
...
static uint8_t osal_find_event( void )
{
    uint8 idx = 0;
...
void osal_idle_task (void)
{
    ...
    if(osal_find_event() == PPlus_SUCCESS)
        return;
