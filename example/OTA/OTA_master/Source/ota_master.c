...
uint8 otaMasterTaskId;

// GAP GATT Attributes
static const uint8 simpleBLEDeviceName[GAP_DEVICE_NAME_LEN] = "CMT OTA Server";
...
// GAP Role Callbacks
static const gapCentralRoleCB_t otaMasterRoleCB =
{
...
static const gapBondCBs_t otaMasterBondCB =
{
  otaMasterPasscodeCB,
...
static void otaMaster_StartDiscoveryService( void )
{
  ...
  LOG("otaMaster_StartDiscoveryService\r\n");
  ...
  GATT_DiscAllPrimaryServices(pdev->conn_hdl ,otaMasterTaskId);
}



static void on_msg_scan(void)
{
  otam_ctx_t* pctx = &m_otam_ctx;
  if(pctx->fsm_state != OTAM_FSM_IDLE)
  {
    LOG("Can't do scan, incorrect state: %d\n", pctx->fsm_state);
    return;
  }
  
  memset(&(pctx->scan_dev_list[0]), 0 ,sizeof(otac_scan_dev_t)*DEFAULT_MAX_SCAN_NUM);
  pctx->scan_dev_num = 0;
...
static void on_msg_connect(otam_cmd_conn_t * pMsg)
{
  ...
  if(pMsg->id != 0xff){
    if(pctx->scan_dev_num <= pMsg->id){
      LOG("ID %d is out of range\n", pMsg->id);
      return;
    }
    if(pctx->fsm_state != OTAM_FSM_IDLE){
      LOG("Error state [%d], will not do connect\n", pctx->fsm_state);
      return;
    }
...
static void on_msg_ota_choose_mode(otam_cmd_mode_t * pMsg)
{
  if(pMsg->mode != OTA_MODE_OTA && pMsg->mode != OTA_MODE_RESOURCE){
    LOG("OTA Choose mode: invalid parameter! \n");
    return;
  }
...
static void on_msg_ota_start(osal_event_hdr_t * pMsg)
{
  otamProtocol_start_ota(0);
}

static void on_msg_ota_stop(osal_event_hdr_t * pMsg)
{
...
static void otaMaster_ProcessCMDMsg(osal_event_hdr_t * pMsg)
{
  switch(pMsg->status){
  case OTAM_CMD_SCAN:
    on_msg_scan();
  ...
  case OTAM_CMD_OTA_START:
    on_msg_ota_start(pMsg);
    break;
  case OTAM_CMD_OTA_STOP:
    on_msg_ota_stop(pMsg);
    break;
  case OTAM_CMD_OTA_REBOOT:
    on_msg_ota_reboot(pMsg);
    break;
  default:
    LOG("Unknow command!\n");
    break;  }
...
{
  osal_set_event(otaMasterTaskId, KEY1_PRESS_EVT);
}
...
static void otaMaster_ProcessOSALMsg( osal_event_hdr_t *pMsg )
{
  switch ( pMsg->event )
  {
    case GATT_MSG_EVENT:
      ...
      break;
    case OTAM_MSG_EVENT:
      otaMaster_ProcessCMDMsg(pMsg);
      break;
  }
}


uint16 otaMaster_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    {
        // Start the Device
        VOID GAPCentralRole_StartDevice( (gapCentralRoleCB_t *) &otaMasterRoleCB );

        // Register with bond manager after starting device
        GAPBondMgr_Register( (gapBondCBs_t *) &otaMasterBondCB );
    ...
    if ( events & KEY1_PRESS_EVT)
    {
      otam_oneclick_ota();
      return ( events ^ KEY1_PRESS_EVT);
    }
...
static void otac_dle_phy_mtu(void)
{
  ...
  ATT_SetMTUSizeMax(DEFAULT_OTAM_MTU_SIZE);
  {
    ...
    uint8 status =GATT_ExchangeMTU(pdev->conn_hdl, &pReq, otaMasterTaskId);
...
static void otac_dev_connected(gapEstLinkReqEvent_t* pEvent)
{
  ...
  osal_start_timerEx(otaMasterTaskId, START_PHY_DEL_MTU_EVT, 20);
...
static void otaMasterEventCB( gapCentralRoleEvent_t *pEvent )
{
  ...
  switch ( pEvent->gap.opcode )
  {
    ...
    default:
    {
      LOG(" otaMasterEventCB --> pEvent->gap.opcode: 0x%02X\r\n", pEvent->gap.opcode);
    }
    break;
  }
}
...
static void otaMasterPairStateCB( uint16 connHandle, uint8 state, uint8 status )
{
  LOG("otaMasterPairStateCB in param state 0x%02X,status 0x%02X\r\n",state,status);
...
void otaMaster_Init( uint8 task_id )
{
