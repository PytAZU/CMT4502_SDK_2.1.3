...
static void handle_ota_notify_event(void* param, uint8_t len)
{
  ...
  case OTA_RSP_PARTITION_COMPLETE:
  {
    ...
    ui_normal_ota_percent((pfw->total_offset*100)/pfw->total_size);
...
int load_fw(uint8_t fw_id)
{
  ...
  for (uint8_t i = 0; i < pfw->part_num; i++){
    pfw->part[i].run_addr = pdata32[i*2+2];
    pfw->part[i].size = pdata32[i*2+3];
    pfw->part[i].flash_addr = faddr + offset;
    pfw->part[i].checksum = crc16(0, (const volatile void * )(pfw->part[i].flash_addr), pfw->part[i].size);
