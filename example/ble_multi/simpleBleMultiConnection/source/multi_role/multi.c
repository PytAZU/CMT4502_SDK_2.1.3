...
#ifdef CLI_ROLE
/// modify for ble cli
uint8 gapMultiRole_TaskID;   // Task ID for internal task/event processing
uint8_t scan_disc_mode = DEFAULT_DISCOVERY_MODE;
uint8_t activeScamode = DEFAULT_DISCOVERY_ACTIVE_SCAN;
uint8_t scanWhiteListMode = DEFAULT_DISCOVERY_WHITE_LIST;
uint8_t highDutyCycleMode = DEFAULT_LINK_HIGH_DUTY_CYCLE;
uint8_t estConnWhiteList = DEFAULT_LINK_WHITE_LIST;
uint16 cli_conn_handle;
uint16_t cli_mtu_size;
#else
...
#ifdef CLI_ROLE
	if( events & ADV_ENABLE_EVT)
	{
		AT_LOG("ADV_ENABLE_EVT\n");
		g_MultiRoleSM.lastState = GAPROLE_INIT;
		g_MultiRoleSM.currentState = GAPROLE_WAITING;
		MultiRole_SM_ReScheduler();
		return ( events ^ ADV_ENABLE_EVT );
	}
	if(events & SCAN_ENABLE_EVT)
	{
		AT_LOG("SCAN_ENABLE_EVT\n");
		g_MultiRoleSM.lastState = GAPROLE_ADVERTISING;
		g_MultiRoleSM.currentState = GAPROLE_WAITING;
		MultiRole_SM_ReScheduler();
		return ( events ^ SCAN_ENABLE_EVT );
	}
	if(events & ESTABLISH_CONN_EVT)
	{
		AT_LOG("ESTABLISH_CONN_EVT\n");
		...
		MultiRole_SM_ReScheduler();
		return ( events ^ ESTABLISH_CONN_EVT );
	}
	if(events & SLAVE_CONN_PARAM_UPDATE)
	{
		AT_LOG("SLAVE_CONN_PARAM_UPDATE\n");
		if( ( g_MultiPeriInfo[cli_conn_handle].paramUpdateEnable == TRUE ) && \
			( g_MultiPeriInfo[cli_conn_handle].paramUpdateState == STATE_IDLE || g_MultiPeriInfo[cli_conn_handle].paramUpdateState == STATE_DONE) && \
			  ...
			  ( 0 == osal_get_timeoutEx( gapMultiRole_TaskID , SLV_CONN_PARAM_TIMEOUT_EVT) )) )
		{
			...
			{
				g_MultiPeriInfo[cli_conn_handle].paramUpdateState = STATE_WAITING;
			}
		}
		return ( events ^ SLAVE_CONN_PARAM_UPDATE );
	}
	if(events & MTU_EXCHANGE_EVT)
	{
		AT_LOG("MTU_EXCHANGE_EVT\n");
		if( g_MultiCentrInfo[cli_conn_handle].CentState == GAPROLE_CONNECTED )
		{
			ATT_SetMTUSizeMax( cli_mtu_size );
			attExchangeMTUReq_t pReq;
			pReq.clientRxMTU = cli_mtu_size;
			uint8 status =GATT_ExchangeMTU(	MultiRole_MSTMatchConnIdx(cli_conn_handle),\
											&pReq, gapMultiRole_TaskID );
			if( status == SUCCESS )
			{
				g_MultiCentrInfo[cli_conn_handle].MUT_ECState = MTUEC_STATE_ING;
			}

			AT_LOG( "handle %d ,[MTU Req]%d %d\n",cli_conn_handle,status,pReq.clientRxMTU);
		}
		return ( events ^ MTU_EXCHANGE_EVT );
	}
#endif
...
static uint8_t MultiRole_processGAPMsg(gapEventHdr_t *pMsg)
{
	...
	//					LOG("GAP Make Discoverable Done idx=%d\n",i);
			...
			else
			{
				...
				g_MultiPeriInfo[0].PeriState = GAPROLE_INIT;///fix adv enable without set adv param bug	
...
#ifdef CLI_ROLE
			AT_LOG("GAP_END_DISCOVERABLE_DONE_EVENT \n");
			// g_MultiRoleSM.lastState = g_MultiRoleSM.currentState;
			// g_MultiRoleSM.currentState = GAPROLE_WAITING;
			// MultiRole_SM_ReScheduler();
#else
...
//				LOG("GAP End Discoverable status 0x%02X \n",pPkt->hdr.status );
		...
		case GAP_LINK_ESTABLISHED_EVENT:
		{
			...
			AT_LOG("GAP_LINK_ESTABLISHED_EVENT \n");
			/// ble cli modify  for single connection
			// g_MultiRoleSM.lastState = g_MultiRoleSM.currentState;
			// g_MultiRoleSM.currentState = GAPROLE_WAITING;
#else
			osal_stop_timerEx(gapMultiRole_TaskID, CONN_TIMEOUT_EVT );
...
//				memset( g_MultiCentrInfo[i].peerAddr , 0, B_ADDR_LEN );
		...
		{
			if(g_multiLinkInfo[i].RoleState == Slave_Role)
			{
				g_MultiCentrInfo[i].initFlag = FALSE ;
				GAPMultiRole_TerminateConnection(i);
			}
		}
#endif
	}
...
uint8 MultiRole_Scanning(void)
{
	LOG("Scan ... \n");
#ifdef CLI_ROLE
	uint8 ret = GAPMultiRole_StartDiscovery( scan_disc_mode,
											 activeScamode, 	 // passive scan
											 scanWhiteListMode );
#else
	...
	{
		AT_LOG("MultiRole_Scanning ret = %d\n",ret);
	}
#endif
...
//					if( 0 == memcmp(&g_MRSDevList[j].addr[0],g_MultiCentrInfo[i].peerAddr,B_ADDR_LEN ) )
					{
#ifdef CLI_ROLE
						ret = GAPMultiRole_EstablishLink(	highDutyCycleMode,
															estConnWhiteList,
...
void MultiRole_PrepareSDP(void)
{
	for(uint8 i=0;i < MAX_CONNECTION_MASTER_NUM; i++)
	{
		if( g_MultiCentrInfo[i].CentState == GAPROLE_CONNECTED )
		{
			...
			// 	if( status == SUCCESS )
			// 	{
					g_MultiCentrInfo[i].MUT_ECState = MTUEC_STATE_ING;/// in order to start sdp
			// 	}
...
#ifdef CLI_ROLE
					HCI_LE_ReadRemoteUsedFeaturesCmd(MultiRole_MSTMatchConnIdx(i));
					HCI_ReadRemoteVersionInfoCmd(MultiRole_MSTMatchConnIdx(i));
#endif
				}
