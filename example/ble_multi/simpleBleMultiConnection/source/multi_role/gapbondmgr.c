    ...
    case GAP_AUTHENTICATION_COMPLETE_EVENT:
      {
		...
		pAuthEvtTmp = pAuthEvt;
		pAuthEvt = pAuthBond[pPkt->connectionHandle];
...
//	AT_LOG("pAuthEvt 0x%p\n",pAuthEvt);
  ...
  { 
    ...
    if ( pAuthEvt == NULL )
    {
      ...
      pAuthEvt = pPkt;
	  // 2020-04-26 add for multi-role
	  pAuthBond[pPkt->connectionHandle] = pAuthEvt;
    }
    else
    {
...
static void gapBondFreeAuthEvt( void )
{
  if ( pAuthEvt != NULL )
  {
	...
	pAuthBond[pAuthEvt->connectionHandle] = NULL;
    pAuthEvt = NULL;
  }
...
static void gapBondRecvEvt(uint16 connHandle , gapBondRec_t *pBondRec, gapAuthCompleteEvent_t *pPkt )
{
//	AT_LOG("gapBondRecvEvt \n");
	gapAuthCompleteEvent_t *pAuthEvtTmp = NULL;
	pAuthEvtTmp = pAuthEvt;
	pAuthEvt = pAuthBond[connHandle];
