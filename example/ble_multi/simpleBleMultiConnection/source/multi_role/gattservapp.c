...
LOG("%s,numPrepareWrites %d\n",__func__,numPrepareWrites);
  ...
  LOG("%s,pQueue %p,queueSize %d\n",__func__,pQueue,queueSize);
  if ( pQueue != NULL )
  {
...
LOG("%s,maxNumPrepareWrites %d\n",__func__,maxNumPrepareWrites);
    return ( SUCCESS );
  }
...
static void gattServApp_ProcessMsg( gattMsgEvent_t *pMsg )
{
  ...
  uint8 status;
	...
	LOG("gattServApp_ProcessMsg method 0X%X\n",pMsg->method);
#if defined ( TESTMODES )
...
LOG("gattServApp_ProcessMsg status %d\n",status);
...
static bStatus_t gattServApp_ProcessExchangeMTUReq( gattMsgEvent_t *pMsg )
{
  ...
  // Send response back
  bStatus_t ret = ATT_ExchangeMTURsp( pMsg->connHandle, pRsp );

  if(ret==SUCCESS)
  {
//      ATT_MTU_SIZE_UPDATE(MIN(pRsp->serverRxMTU, pMsg->msg.exchangeMTUReq.clientRxMTU));
	  ATT_UpdateMtuSize(pMsg->connHandle, MIN(pRsp->serverRxMTU, pMsg->msg.exchangeMTUReq.clientRxMTU));
  }  
...
LOG("gattServApp_ProcessReadByGrpTypeReq \n");
  // Only the attributes with attribute handles between and including the
  // Starting Handle and the Ending Handle with the attribute type that is
  // the same as the Attribute Type given will be returned.
...
static bStatus_t gattServApp_ProcessExecuteWriteReq( gattMsgEvent_t *pMsg, uint16 *pErrHandle )
{
  ...
  LOG("%s,pQueue %p maxNumPrepareWrites %d\n",__func__,pQueue,maxNumPrepareWrites);
  if ( pQueue != NULL )
  {
...
static bStatus_t gattServApp_EnqueuePrepareWriteReq( uint16 connHandle, attPrepareWriteReq_t *pReq )
{
  ...
  if ( pQueue != NULL )
  {
    ...
    for ( uint8 i = 0; i < maxNumPrepareWrites; i++ )
    {   
        LOG("pQueue->pPrepareWriteQ[%d].handle %d\n",i,pQueue->pPrepareWriteQ[i].handle);
