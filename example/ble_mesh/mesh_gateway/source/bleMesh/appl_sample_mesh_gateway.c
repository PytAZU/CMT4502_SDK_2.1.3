...
//        ("Send Vendor Model Set\n");
        ...
        if(retval == API_SUCCESS)
        {
            ...
            set_param.phy_mode_type = MS_STATE_PHY_MODEL_ONOFF_T;
            switch(set_param.phy_mode_type)
            {
                case MS_STATE_PHY_MODEL_ONOFF_T:
                {
                    set_param.phy_mode_param = EM_alloc_mem(1);
                    set_param.phy_mode_param[0] = (g_buttonCounter++)&0x01;
                }
            }

            retval=MS_phymodel_client_send_reliable_pdu(MS_ACCESS_PHY_MODEL_SET_OPCODE,&set_param,0,dst_addr);
            if(NULL != set_param.phy_mode_param)
            {
            ...
            marker += 2;
            set_param.phy_mode_param = &buffer[0];

            retval=MS_phymodel_client_send_reliable_pdu(MS_ACCESS_PHY_MODEL_SET_OPCODE,&set_param,dst_addr);
#endif            
