...
#define BLEMESH_COMN_TICK_EVT                            	 0x0020 
#define BLEMESH_COMN_TICK_TOUT                            100 
#define BLEMESH_LIGHT_TIME_ON_EVT                        	 0x0100//change by johhn
#define BLEMESH_LIGHT_TIME_OFF_EVT                         0x0200//change by johhn
...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "CMOSTEK ALI MSH";
...
//    hciStatus_t ret;
    ...
    if ( events & BLEMESH_START_DEVICE_EVT )
    {
		...
		osal_set_event( bleMesh_TaskID, BLEMESH_COMN_TICK_EVT );//ppsp_impl_appl_timr_hdlr

       return ( events ^ BLEMESH_START_DEVICE_EVT );
    }
		 if ( events & BLEMESH_COMN_TICK_EVT )
		 {
				//ppsp_impl_appl_timr_hdlr();
			 return ( events ^ BLEMESH_COMN_TICK_EVT );
    }
...
//    	cry_ecdh_process_secret();
		...
		if (events & BLEMESH_LIGHT_TIME_ON_EVT)
    {
			generic_onoff_set_pl(1);
     	return (events ^ BLEMESH_LIGHT_TIME_ON_EVT);
		}
		if (events & BLEMESH_LIGHT_TIME_OFF_EVT)
    {
			generic_onoff_set_pl(0);
     	return (events ^ BLEMESH_LIGHT_TIME_OFF_EVT);
		}
