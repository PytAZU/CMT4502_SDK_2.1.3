...
{ 
	MS_ACCESS_MODEL_HANDLE	model_handle[MODEL_TREE_MAXNUM];
    MS_ACCESS_ADDRESS		sub_addr;
    sub_addr.use_label=0;
    sub_addr.addr=addr;
	API_RESULT ret = API_FAILURE;
	get_model_handles_of_element(model_handle,element_handle);

    MS_access_ps_store_disable(1);
	

	for(int i = 0; i < MODEL_TREE_MAXNUM; i++)
	{
	  ret = MS_access_cm_add_model_subscription(model_handle[i],&sub_addr);
		DEBUG_PRINT("[SUBSCRIPTION_ADD subaddr = 0x%04X to model_handle = 0x%04X, ret = 0x%04X\n]",addr,model_handle[i],ret);
...
{  
	...
	for(int i = 0; i < MODEL_TREE_MAXNUM; i++)
	{
		ret = MS_access_cm_delete_model_subscription(model_handle[i],&sub_addr);
		DEBUG_PRINT("[SUBSCRIPTION_DEL subaddr = 0x%04X from model_handle = 0x%04X, ret = 0x%04X\n]",addr,model_handle[i],ret);
	}
