...
API_RESULT UI_generic_onoff_model_state_set(UI_DATA_GENERIC_ONOFF_MODEL_T * me, MS_ACCESS_MODEL_REQ_MSG_RAW * msg_raw)
{
	...
	if(msg_raw->data_len == 4)
	{
		
	    me->UI_state_generic_onoff.transition_time 	= msg_raw->data_param[2];
		me->UI_state_generic_onoff.delay 			= msg_raw->data_param[3];
		
	
	    me->UI_state_generic_onoff.transition_time 	= calc_trans_time(msg_raw->data_param[2]) | EM_TIMEOUT_MILLISEC;
		me->UI_state_generic_onoff.delay		= ((((UINT32)msg_raw->data_param[3]) & 0x000000FF) * 5 ) | EM_TIMEOUT_MILLISEC;
	

		
	    DEBUG_PRINT("[ONOFF_SET] transition_time_ms: 0x%02X\r\n", me->UI_state_generic_onoff.transition_time);
	    DEBUG_PRINT("[ONOFF_SET] delay_time_ms: 0x%02X\r\n", me->UI_state_generic_onoff.delay);

	
	
		if(me->UI_state_generic_onoff.delay > 0)
		{
			EM_start_timer (&(me->tmrhdl_delay), me->UI_state_generic_onoff.delay, UI_ACTION_generic_onoff_delay_cb, &me, sizeof(UI_DATA_GENERIC_ONOFF_MODEL_T *));
		}
		else if (me->UI_state_generic_onoff.transition_time> 0)
		{
			EM_start_timer (&(me->tmrhdl_trans), me->UI_state_generic_onoff.transition_time, UI_ACTION_generic_onoff_delay_cb, &me, sizeof(UI_DATA_GENERIC_ONOFF_MODEL_T *));
		}
		else
		{
			me->UI_state_generic_onoff.onoff = me->UI_state_generic_onoff.target_onoff;
                  ...
                  )
{
    ...
    if (0x01 == req_type->to_be_acked)
    {
		...
		while(1)
		{
			...
			{
				//ERROR_PRINT("Generic Onoff ack Failed. Result: 0x%04X\n",retval);
			}

		}
		
		vendor_Aligenie_indication( ctx->saddr,0x0100,me->UI_state_generic_onoff.target_onoff,attr_len_def[BOOL]);
