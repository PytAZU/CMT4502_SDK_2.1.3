...
static hal_spi_t lcd_spi_handle = {
	.spi_index = SPI0,
};
extern hal_spi_t spi;

extern int hal_spi_module_select(hal_spi_t* spi_ptr);
void select_lcd(void)
{
	hal_spi_module_select(&lcd_spi_handle);
}

void select_flash(void)
{
	hal_spi_module_select(&spi);
}
...
extern void spi_gd25q16_read_test_set_buffer(bool cs_force,uint32_t addr,uint8_t* tx_buf,uint8_t* rx_buf,uint16_t transmit_len);

int lcd_setscn_img_use_double_spi(int index)
{
	//uint8_t temp;
	uint8_t transmit_len;
	uint32_t size = 1152,addr_to_flash;//,i=0;
//	void* image_addr = ((void*)0x11038000);

  set_region(0, 0, 239, 239);
  hal_gpio_write(DC_PIN,1);
	{
		addr_to_flash = 1024*128*index;
		size=115200;
		while(size)
		{
				uint16_t size_buf = size > 8 ? 8: size;
				...
				select_flash();
				//GD25_read(addr_to_flash,(uint8_t*)rx_buf,8);	
  ...
  for(j = 0; j< h; j++){
    for(i = 0; i< w; i++){
      if(((x + i)>xe)||((y+j)>ye))
        continue;
      val = data[j*w+i];
      hal_spi_Tx(&lcd_spi_handle,(uint8_t)(val>>8));
			//hal_spi_Tx(&lcd_spi_handle,val&0xff);
      hal_spi_Tx(&lcd_spi_handle,(uint8_t)val);
    }
  }
  hal_spi_TxComplete(&lcd_spi_handle);
  return 0;
...
int lcd_bus_init(void)
{
  int ret;
    ...
    //ret = hal_spi_bus_init(&lcd_spi_handle,cfg);//spi init
		if(hal_spi_bus_init(&lcd_spi_handle,cfg) != PPlus_SUCCESS)
		{
			while(1);;;
		}
...
//lcd_init:read spif data display on lcd
//read_flash_display:read gd25q16 data display on lcd
void read_gd25q16_flash_display_lcd(void){
  ...
  lcd_setscn_TFT(200,0,16,200,color);
	...
	while(1)
	{
		lcd_setscn_img_use_double_spi(0);//use two spi
		//lcd_setscn_img(0);
		select_lcd();
		lcd_on_TFT();
		WaitMs(500);
		
		lcd_setscn_img_use_double_spi(1);//use two spi
		//lcd_setscn_img(1);
		select_lcd();
		lcd_on_TFT();
		WaitMs(500);
