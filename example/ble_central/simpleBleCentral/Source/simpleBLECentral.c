...
gapDevRec_t simpleBlePeerTarget; 
#else
...
#ifdef CLI_ROLE
    ///cli process
    if ( events & CLI_UART_RX_EVT)
    {
        if(cmdstr[cmdlen-1]=='\r')
        {
            cmdstr[cmdlen - 1] = '\0';
            AT_LOG("cmdstr[0] = %d\r\n", cmdstr[0]);
            AT_LOG("cmdstr = %s\r\n", cmdstr);
...
//                uint16 phyOption = 0x00;
	...
	if ( events & SBC_PERIODIC_EVT )
    {
        ...
        for(int i=0;i<centralTestVectorTbl[cTVIdx].notfPktNum;i++)
        {
            ...
            if(status==SUCCESS)
            {
                T0 = read_current_fine_time()-T0;
                LOG("[WTNR_TX] %02x %x L%d %dus \n",status,mstWtCnt,pReq.len,T0);
                mstWtCnt++;
...
//				osal_stop_timerEx(simpleBLETaskId, SBC_CANCEL_CONN);
                ...
                if(centralTestVectorTbl[cTVIdx].mtu>23)
                {
                    ...
                    AT_LOG( "[MTU Req]%d %d simpleBLEConnHandle = %d\n",status,pReq.clientRxMTU,simpleBLEConnHandle);
...
{
    volatile uint32 uOS_size,uOS_cnt;
       ...
       ntfTest.isDone==0)
    {
        ...
        //osal_memory_statics(NULL, &uOS_cnt,&uOS_size);
        HCI_PPLUS_ConnEventDoneNoticeCmd(simpleBLETaskId, NULL);
        LOG("[TVEC] %08x case %3d/%d[cnt %6d miss %2d err %2d] [%04x %04x]\n",getMcuPrecisionCount()
                                                                          ,cTVIdx
        ...
        LOG ("[TVEC] mAloc[ll%4x r%4x s%4x] SAR[r%4x %4x %4x %4x s%4x %4x %4x %4x ]\n"
                                                                                ,g_pmCounters.ll_hci_buffer_alloc_err_cnt
        ...
        {
            LOG("[TVEC] %08x >>>>>>>>> ALL CASE FINISHED Total %d Err %d <<<<<<<<<\n",getMcuPrecisionCount(),cTVIdx,cTVErrCnt);
            cTVIdx=0;
//            while(1){};
