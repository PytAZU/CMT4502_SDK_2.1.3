...
static void hal_init(void)
{
  ...
  LOG_INIT();
	...
	//config XIP performance
	*((unsigned int*)0x1108008c) = 3;
	loop();
	reg = read_reg(0x1108008c);
	LOG("0x1108008c is %x\n", reg);
//	reg &= ~(12ul);
//	reg |= 0;
//	write_reg(0x1108008c, reg);
//	reg = read_reg(0x1108008c);
//	LOG("0x1108008c modified %x\n", reg);
  hal_gpio_init();
  hal_adc_init();
...
static int hal_gpio_write_ram(GPIO_Pin_e pin, uint8_t en) {
...
static void xip_test(void)
{
  ...
  hal_gpio_write_ram(P15, 0);
  for(i = 0; i< 10; i++){
    hal_gpio_write_ram(P14, 0);
    hal_gpio_write_ram(P14, 1);
    loop();
    hal_gpio_write_ram(P14, 0);
    hal_gpio_write_ram(P14, 0);
    hal_gpio_write_ram(P14, 0);
