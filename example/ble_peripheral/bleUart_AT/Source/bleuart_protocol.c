...
//uint8_t gRxIdleTimeout = 3;

//uint8_t ptcmdstr[AT_CMD_PT_LEN_MAX] = {0};
//uint8_t ptcmdlen = 0;
...
void gpio_sleep_handle()
{
	  //hal_gpio_write(FLOW_CTRL_IO_BLE_TX,1);
	  //hal_gpio_write(FLOW_CTRL_IO_BLE_TX,0);
	
		hal_gpio_pin_init(FLOW_CTRL_IO_HOST_WAKEUP, IE);
		hal_gpio_pin_init(P9, IE);
		hal_gpio_pull_set(P9, FLOATING);	  
		hal_gpio_fmux(FLOW_CTRL_IO_HOST_WAKEUP, Bit_DISABLE);      //enable fullmux fuction; enable or disable 
		hal_gpio_wakeup_set(FLOW_CTRL_IO_HOST_WAKEUP, NEGEDGE); //  NEGEDGE
}

void gpio_wakeup_handle()
{
	int i,flag = 0;
	uint8_t ret = 0;
			    ...
			    //hal_gpio_write(P15,1);
				  //hal_gpio_write(P15,0);
					...
					if(get_uart_at_mod()){
						...
						at_Init();
			      AP_UART0->LCR = 0x0b; //8bit, 1 stop odd parity TEST
						osal_start_timerEx( bleuart_TaskID, BUP_OSAL_EVT_AT_AUTO_SLEEP, m_auto_slp_time * 1000 );
					}
					else{
						BUP_connect_handler();
			      AP_UART0->LCR = 0x1b; //8bit, 1 stop odd parity TEST
						BUP_init();
					}
					break;
				}
  ...
  if(pctx->rx_state != BUP_RX_ST_IDLE && pctx->rx_size)
  {
    ...
    while(1){
      ...
      attHandleValueNoti_t notify_data={0};
			...
			HAL_ENTER_CRITICAL_SECTION();
      size = ((pctx->rx_size - pctx->rx_offset) > 20) ? 20 : pctx->rx_size - pctx->rx_offset; //ATT_MTU_SIZE  20  192
