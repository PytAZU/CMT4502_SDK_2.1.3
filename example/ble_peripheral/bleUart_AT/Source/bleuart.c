...
static uint16 company_ID = 0x4502; 
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "cmt_01";
...
void bleuart_gen_scanRspData(uint8_t *name, uint16_t *cint, uint8_t rf_pw)
{
	...
	switch(rf_pw){
		case 0:
			real_rfpw = RF_PHY_TX_POWER_4DBM;  //RF_PHY_TX_POWER_4DBM. <rf_phy_driver.h>
			break;
		case 1:
			real_rfpw = RF_PHY_TX_POWER_0DBM;  //RF_PHY_TX_POWER_0DBM
			break;
		case 2:
			real_rfpw = RF_PHY_TX_POWER_N6DBM;  //RF_PHY_TX_POWER_N6DBM
			break;
		case 3:
		...
		case 4:
			...
			break;
		default:
			real_rfpw = RF_PHY_TX_POWER_4DBM;  //use default power value in case not valid. Should not happen.
	}
...
/* Skip these events as they are not used at the moment. 
		...
		case BUP_OSAL_EVT_UART_DATA_RX:{ // 
