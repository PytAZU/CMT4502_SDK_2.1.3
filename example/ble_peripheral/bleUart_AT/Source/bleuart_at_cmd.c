...
uint8_t g_adv_data[AT_ADV_DATA_SIZE] = "cmt_01"; 
...
// new added.
//uint16_t at_it(uint32_t argc, uint8_t *argv[]);
 ...
 **/
	  ...
	  if(argc == 1){
			if('?' == argv[0][0]){
				...
				switch(argv[0][0]){
					case '0':
					  mAT_Ctx.rfpw = 0;  //RF_PHY_TX_POWER_4DBM. <rf_phy_driver.h>
					...
					case '1':
					  ...
					  break;
					case '2':
					  mAT_Ctx.rfpw = 2;  //RF_PHY_TX_POWER_N6DBM
...
{
	  //uint8_t ret = 0;
	  int8_t idletimeout = 0;
	  uint8_t i = 0;
		uint8_t data_str[4] = {0}; // 4 bytes should be enough for 8-bit data.
	  if(argc == 1){
			if('?' == argv[0][0]){
				idletimeout = get_rx_idle_timeout();
				AT_LOG("\n%d",idletimeout);
			}
			else{
				//check whether the data string is valid or not.
				for( i=0; ((argv[0][i]) != '\0'); i++){
					if( IS_DIGIT(argv[0][i]) ) // is valid data number.
						...
						goto ERR_exit;
					if( i >= 3 ) // too long data string.
						goto ERR_exit;
				}
				data_str[i+1] = '\0';
				
				idletimeout = CLI_strtoi(data_str, strlen((char *)data_str),10);
				if(idletimeout > 100)
					idletimeout = 100;
				set_rx_idle_timeout(idletimeout);
			}
			AT_LOG("\nOK\n");
			return 0;
		}
